...
class LeakGAN_G_2LSTM(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, vocab_size, max_seq_len, padding_idx, goal_size,
        ...
        self.worker = nn.GRU(embedding_dim, hidden_dim)
        self.manager = nn.GRU(goal_out_size, hidden_dim)
    ...
    def forward_leakgan(self, sentences, dis, if_sample, no_log=False, start_letter=cfg.start_letter, train=False):
        ...
        work_hidden = self.init_hidden_gru(batch_size) #Comment out if using GRU
        #mana_hidden = self.init_hidden(batch_size)
        mana_hidden = self.init_hidden_gru(batch_size) #Comment out if using GRU
    ...
    def init_hidden_gru(self, batch_size=1):
